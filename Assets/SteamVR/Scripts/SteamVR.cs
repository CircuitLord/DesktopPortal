...
namespace Valve.VR {
	public partial class SteamVR : System.IDisposable {
		...
		private static string GetManifestFile() {
			...
			if (File.Exists(fullPath)) {
				...
				SteamVR_Input_ManifestFile existingFile = JsonUtility.FromJson<SteamVR_Input_ManifestFile>(jsonText);
			...
			if (File.Exists(fullPath) == false) {
				...
				manifestApplication.strings.Add("en_us",
					new SteamVR_Input_ManifestFile_ApplicationString() {name = string.Format("{0} [Testing]", Application.productName)});
		...
		private static void IdentifyEditorApplication(bool showLogs = true) {
			...
			if (applicationIdentifyErr != EVRApplicationError.None)
				...
				if (showLogs)
					Debug.Log(
						string.Format("<b>[SteamVR]</b> Successfully identified process as editor project to SteamVR ({0})", SteamVR_Settings.instance.editorAppKey));
			}
		}
