...
public class MonitorTexture : MonoBehaviour
{
    UDDMonitor _uddMonitor;
    public UDDMonitor uddMonitor 
    { 
        get { return _uddMonitor; }
        set 
        { 
            _uddMonitor = value;
            if (_uddMonitor != null) {
                material.mainTexture = _uddMonitor.texture;
                width = transform.localScale.x;
                rotation = uddMonitor.rotation;
                invertX = invertX_;
                invertY = invertY_;
                useClip = useClip_;
            }
    ...
    { 
        get { return uddMonitor.id; }
        set { uddMonitor = UDDManager.GetMonitor(value); }
    }
    ...
    {
        if (uddMonitor == null) {
            uddMonitor = UDDManager.primary;
        }
        UDDManager.onReinitialized += Reinitialize;
    }
    ...
    public Vector3 GetWorldPositionFromCoord(int u, int v)
    {
        ...
        var x =  (float)(u - uddMonitor.width  / 2) / uddMonitor.width;
        var y = -(float)(v - uddMonitor.height / 2) / uddMonitor.height;
    ...
    {
        public bool hit;
        public MonitorTexture MonitorTexture;
    ...
    public RayCastResult RayCast(Vector3 from, Vector3 dir)
    {
        ...
        // Desktop position
        int desktopX = uddMonitor.left + (int)((coordX + 0.5f) * uddMonitor.width);
        int desktopY = uddMonitor.top + (int)((0.5f - coordY) * uddMonitor.height);
    ...
    {
        foreach (var uddTexture in GameObject.FindObjectsOfType<uDesktopDuplication.MonitorTexture>()) {
