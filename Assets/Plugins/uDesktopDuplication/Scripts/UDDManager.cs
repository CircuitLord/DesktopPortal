...
{
    private static UDDManager instance_;
    public static UDDManager instance 
    {
    ...
    private List<UDDMonitor> monitors_ = new List<UDDMonitor>();
    static public List<UDDMonitor> monitors
    {
    ...
    static public UDDMonitor primary
    {
        get 
        {
    ...
    public static UDDMonitor GetMonitor(int id)
    {
        if (id < 0 || id >= UDDManager.monitors.Count) {
            Debug.LogErrorFormat("[uDD::Error] there is no monitor whose id is {0}.", id);
            return UDDManager.primary;
        }
        return monitors[Mathf.Clamp(id, 0, UDDManager.monitorCount - 1)];
    }
