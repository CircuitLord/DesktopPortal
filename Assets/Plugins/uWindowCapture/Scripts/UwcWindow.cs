...
public class UwcWindow
{
    ...
    public void PopulateFriendlyTitle(bool force = false) {

        if (friendlyTitle != "" && !force) return;
        
        string fileDesc = GetFileDescForPID(processId);

        if (fileDesc != null) {
            
            //UWP Application:
            if (fileDesc.StartsWith("Application Frame")) {
                friendlyTitle = title;
            }
            else {
                friendlyTitle = fileDesc;
            }
        }
        else {
            friendlyTitle = title;
    ...
    private string GetFileDescForPID(int pid) {
        string fileDesc = "";
        string path = "";
    ...
    public bool GetHasNewTex()
    {
	    return Lib.GetHasNewTex(id);
    }
    
    public void SetHasNewTex(bool state)
    {
	    Lib.SetHasNewTex(id, state);
    }
